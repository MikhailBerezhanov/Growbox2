...
function check_for_input(id)
{
    if(document.getElementById(id).value == "") return document.getElementById(id).getAttribute("placeholder");

    return document.getElementById(id).value;
}

function create_time_json(type)
{
    var hour_value = check_for_input(type + "_hour"); 
    var min_value = check_for_input(type + "_min");
    var sec_value = check_for_input(type + "_sec");

    return (JSON.stringify({ "hour": Number(hour_value),
                             "min": Number(min_value),
                             "sec": Number(sec_value) }) );
}
